...
### --- 4. DEFINIÇÃO DA FUNÇÃO DE FITNESS (ATUALIZADA) ---
# MUDANÇA 1: Aumentar épocas e adicionar EarlyStopping
...
# MUDANÇA 2: Valores reais para otimização
n_agentes = 5
n_iteracoes = 10
lb = [0.0001, 32]
ub = [0.01, 128]
...
print("\n" + "=" * 50, flush=True)
print("--- INICIANDO OTIMIZAÇÃO DE HIPERPARÂMETROS (WSA) ---", flush=True)
print(f"Configuração: {n_agentes} agentes, {n_iteracoes} iterações, máx {EPOCHS_FOR_OPTIMIZATION} épocas.", flush=True)
print("=" * 50 + "\n", flush=True)
...
print("\nA obter a melhor solução encontrada...", flush=True)
best_params_wsa = wsa_optimizer.get_Gbest()

# Verifica se o método existe (caso esteja a usar o wsa antigo)
if hasattr(wsa_optimizer, 'get_Gbest_fitness'):
    best_fitness_wsa = wsa_optimizer.get_Gbest_fitness()
    print("(Score recuperado da memória do otimizador)")
else:
    print("(Atenção: A re-treinar para confirmar score - atualize o wsa.py para evitar isto)")
    best_fitness_wsa = fitness_function(best_params_wsa)

print("\n" + "=" * 50, flush=True)
print("--- OTIMIZAÇÃO WSA CONCLUÍDA ---", flush=True)
print(f"🏆 Melhor Val Loss (WSA): {best_fitness_wsa:.5f}", flush=True)
print(f"🔩 Melhores Hiperparâmetros (WSA):")
print(f"Learning Rate: {best_params_wsa[0]:.6f}", flush=True)
print(f"Neurónios: {int(best_params_wsa[1])}", flush=True)
print("=" * 50 + "\n", flush=True)
